...
users.forEach(function(e){
    card += `<div class="card">
    <div class="rate">
                <button>${e.available ? "available" :  "" }</button>
                <h3>$${e.rate}/hr</h3>
            </div>
            <div class="image">
                <img class="img-top" src="${e.profilePhoto}" alt="">
            </div>
            <div class="name">
                <h1>${e.fullName}</h1>
            </div>
            ...
            <div class="skills">
            ${e.skills.slice(0, 3).map(skill => `<button>${skill}</button>`).join('')}
            ${e.skills.length > 3 ? `<button style="background-color: #55A8FD; color: white">+${e.skills.length - 3}</button>` : ''}
            </div>
